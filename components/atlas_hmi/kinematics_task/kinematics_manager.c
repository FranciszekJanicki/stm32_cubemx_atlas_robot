...
static atlas_err_t kinematics_manager_event_data_handler(
    kinematics_manager_t* manager,
    kinematics_event_payload_data_t const* data)
{
    ...
    switch (data->type) {
        case ATLAS_DATA_TYPE_CARTESIAN: {
            ...
            event.payload.data.payload.joints_data =
                kinematics_manager_cartesian_to_joints_data(&data->payload.cartesian_data);
        }
        case ATLAS_DATA_TYPE_JOINTS: {
            event.payload.data.type = ATLAS_DATA_TYPE_CARTESIAN;
            event.payload.data.payload.cartesian_data =
                kinematics_manager_joints_to_cartesian_data(&data->payload.joints_data);
        }
        default: {
            event.payload.data.type = ATLAS_DATA_TYPE_NONE;
        }
    }
...
static atlas_err_t kinematics_manager_event_path_handler(
    kinematics_manager_t* manager,
    kinematics_event_payload_path_t const* path)
{
    ...
    system_event_t event = {.type = SYSTEM_EVENT_TYPE_PATH,
                            .origin = SYSTEM_EVENT_ORIGIN_KINEMATICS};

    switch (path->type) {
        case ATLAS_PATH_TYPE_CARTESIAN: {
                                                    ...
                                                    kinematics_event_t const* event)
{
    ...
    switch (event->type) {
        ...
        case KINEMATICS_EVENT_TYPE_DATA: {
            return kinematics_manager_event_data_handler(manager, &event->payload.data);
        }
        case KINEMATICS_EVENT_TYPE_PATH: {
            return kinematics_manager_event_path_handler(manager, &event->payload.path);
        }
        default: {
