...
static inline bool button_manager_receive_button_event(button_event_t* event)
{
    ...
    return xQueueReceive(queue_manager_get(QUEUE_TYPE_BUTTON),
                         event,
                         pdMS_TO_TICKS(1)) == pdPASS;
}
...
{
    return uxQueueMessagesWaiting(queue_manager_get(QUEUE_TYPE_BUTTON)) ==
           pdPASS;
}
...
static atlas_err_t button_manager_notify_handler(button_manager_t* manager,
...
static atlas_err_t button_manager_event_start_handler(
...
static atlas_err_t button_manager_event_stop_handler(
    button_manager_t* manager,
    ...
    button_event_payload_button_data_t const* button_data)
{
    ...
    if (button_data->state == BUTTON_STATE_PRESSED) {
        bool is_long_press =
            (current_tick - manager->button_ctxs[button_data->type].tick) >
            pdMS_TO_TICKS(BUTTON_LONG_PRESS_MS);

        event.payload.button_data.press =
            is_long_press ? BUTTON_PRESS_LONG : BUTTON_PRESS_SHORT;
    } else {
                                                ...
                                                button_event_t const* event)
{
    ...
    switch (event->type) {
        case BUTTON_EVENT_TYPE_START: {
            return button_manager_event_start_handler(manager,
                                                      &event->payload.start);
        }
        case BUTTON_EVENT_TYPE_STOP: {
            return button_manager_event_stop_handler(manager,
                                                     &event->payload.stop);
        }
        case BUTTON_EVENT_TYPE_BUTTON_DATA: {
            return button_manager_event_button_data_handler(
                manager,
