...
static inline bool display_manager_receive_display_event(display_event_t* event)
{
    ...
    return xQueueReceive(queue_manager_get(QUEUE_TYPE_DISPLAY),
                         event,
                         pdMS_TO_TICKS(1)) == pdPASS;
}
...
{
    return uxQueueMessagesWaiting(queue_manager_get(QUEUE_TYPE_DISPLAY)) ==
           pdPASS;
}
...
static atlas_err_t display_manager_event_start_handler(
...
static atlas_err_t display_manager_event_stop_handler(
    display_manager_t* manager,
                                                 ...
                                                 display_event_t const* event)
{
    ...
    switch (event->type) {
        ...
        case DISPLAY_EVENT_TYPE_ROBOT_DATA: {
            return display_manager_event_robot_data_handler(
                manager,
                &event->payload.robot_data);
        }
        case DISPLAY_EVENT_TYPE_ROBOT_PATH: {
            return display_manager_event_robot_path_handler(
                manager,
